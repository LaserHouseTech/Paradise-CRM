...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Initial Supabase health & readiness check on app startup
  useEffect(() => {
    supabaseSyncService.checkStatus().then((res) => {
      if (res.connected && res.tablesFound.length > 0) {
        setSupabaseSyncState('synced');
        setSupabaseSyncMessage(`Supabase online (${res.tablesFound.length} tabelas sincronizadas)`);
      } else if (res.connected && res.missingTables.length > 0) {
        setSupabaseSyncState('error');
        setSupabaseSyncMessage('Aguardando execução do script SQL no Supabase');
      } else {
        setSupabaseSyncState('idle');
      }
    }).catch(() => {
      setSupabaseSyncState('error');
      setSupabaseSyncMessage('Falha ao conectar com o Supabase');
    });
  }, []);
