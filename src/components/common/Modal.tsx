      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
          ...
          <motion.div
            ...
            className={`relative w-full ${maxWidthClasses[maxWidth]} bg-[#0c0f18]/95 backdrop-blur-2xl border border-white/[0.12] rounded-2xl shadow-2xl shadow-black/90 overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[90vh] ring-1 ring-white/10 z-10`}
          ...
          >
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-white/[0.08] flex items-center justify-between gap-3 shrink-0 bg-white/[0.02]">
              <div className="min-w-0 flex-1">
                ...
                {subtitle && <p className="text-[11px] sm:text-xs text-neutral-400 mt-0.5 truncate">{subtitle}</p>}
              </div>
              <button
                onClick={onClose}
            ...
            {/* Content Body */}
            <div className="p-3.5 sm:p-6 overflow-y-auto overflow-x-hidden flex-1">{children}</div>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
